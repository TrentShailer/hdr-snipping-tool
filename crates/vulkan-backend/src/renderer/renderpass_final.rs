...
mod fragment_shader {
    vulkano_shaders::shader! {
        ty: "fragment",
        bytes: "shaders/fragment/final.spv"
    }
}
mod vertex_shader {
    vulkano_shaders::shader! {
...
pub struct RenderpassFinal {
    pub pipeline: Arc<GraphicsPipeline>,
    pub attachment_set: Arc<PersistentDescriptorSet>,
}
...
impl RenderpassFinal {
    pub fn new(
        ...
        let attachment_set =
            Self::recreate_attachment_set(&instance, pipeline.clone(), attachments)?;
...
pub enum Error {
    ...
    #[error("Failed to create attachment set:\n{0:?}")]
    AttachmentSet(#[from] attachment::Error),
}
